...
S.extProto(S.Helpers,{
	...
	createMenu:function(options,links,type){
		...
		return options.tagName==='ul' ? ul : S.createElt(options.tagName).attrs(options.menuAttributes).html(ul);
	...
	linkMenu:function(title,url,linkoptions,options,lioptions){
		...
		if(linkoptions!=null) res=S.createElt('li').setAttrs(lioptions).html(res);
		return res;
	}
});